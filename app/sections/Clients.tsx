...
export default function Clients() {
  ...
  // --- CTA backend integration (from clients.tsx) ---
  const handleAction = async (action: string) => {
    setIsSubmitting(true)
    try {
      const res = await fetch('/api/clients-action', {
        method: 'POST',
          ...
          <div className="bg-lugha-primary rounded-2xl p-8 md:p-12 text-white">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button
                ...
                className="bg-white text-lugha-primary px-8 py-4 rounded-xl font-semibold hover:bg-gray-50 transition-colors duration-300 shadow-lg disabled:opacity-60"
              >
                {isSubmitting ? "Submitting..." : "Get Started"}
              </button>
              <button
                onClick={() => handleAction("View Case Studies")}
                disabled={isSubmitting}
                className="border-2 border-white text-white px-8 py-4 rounded-xl font-semibold hover:bg-white hover:text-lugha-primary transition-colors duration-300 disabled:opacity-60"
              >
                {isSubmitting ? "Submitting..." : "View Case Studies"}
              </button>
            </div>
          </div>
        </div>
